...
def run(sta_path, file, results_path, agg_path, server):
    ...
    def euclidean_heuristic(traj, M): 
        ...
        for idx, row in df.iterrows():
            ...
            dist = getDistanceFromLatLonInm(starting_loc[1], starting_loc[0], row['Latitude'], row['Longitude'])
            ...
            if dist < prev_dist:
                try:
                    ...
                    if(prev_cons_dist+cons_dist > shortest_distance*M):
                        if(prev_cons_dist+cons_dist < 20 or prev_tot_speed < 20 or prev_tot_time_diff < 100):
    ...
    for M in Ms:
        ...
        for trip in tqdm(trips):
            ...
            new_traj, failed_trips, sub_hit, sub_miss, sub_trip_heads = euclidean_heuristic(trip_df, 1+(M/100))
        ...
        if len(new_trajs) > 1:
            ...
            md = f"{M}%"
            e_p = results_path + file+f"-Euclidean-{md}"
            endf_without_short_trips.reset_index(drop=True).to_csv(e_p, compression="xz")
            e_extra_seg.reset_index(drop=True).to_csv(e_p+"-extra", compression="xz")

    with open(agg_path + file+f"-Euclidean-properties.pk", 'wb') as f:
