...
def run(sta_path, file, results_path, agg_path, server):
    ...
    for M in Ms:
        ...
        for trip in tqdm(trips):
            ...
            new_traj, failed_trips, sub_hit, sub_miss, sub_trip_heads = bruteforce(trip_df, 1+(M/100))
        ...
        if len(new_trajs) > 1:
            ...
            md = f"{M}%"
            e_p = results_path + file+f"-Scan-{md}"
            endf.to_csv(e_p, compression="xz")


    with open(agg_path + file+f"-Scan-properties.pk", 'wb') as f:
